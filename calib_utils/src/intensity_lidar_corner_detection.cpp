...
#define M_PI 3.14159265358979323846
                        ...
                        const Eigen::Vector2d &center_reg, double lambda_center)
    {
        ...
        for (int i = 0; i < transformed.rows(); ++i)
        {
            ...
            if (model_color == -1)
            {
                ...
                if (px < model_min_x)
                    dx = model_min_x - px;
                else if (px > model_max_x)
                    dx = px - model_max_x;
                if (py < model_min_y)
                    dy = model_min_y - py;
                else if (py > model_max_y)
                    dy = py - model_max_y;
        ...
        int num_squares_x, int num_squares_y)
    {
        ...
        auto periodicityScore = [&](const Eigen::VectorXd &h, double range) -> double
        {
            ...
            {
                double s = 0;
                for (int k = 0; k + lag < h.size(); ++k)
                    s += mean_sub(k) * mean_sub(k + lag);
                ac(lag) = s;
            }
        ...
        for (double a = -10.0 * M_PI / 180.0; a <= 10.0 * M_PI / 180.0; a += 1.0 * M_PI / 180.0)
            angle_candidates.push_back(a);
        ...
        int internal_corners_x, int internal_corners_y, double checker_size_m)
    {
        ...
        for (int i = 0; i < pts_pca_2d.rows(); ++i)
        {
            ...
            if (x >= min_x_c && x <= max_x_c && y >= min_y_c && y <= max_y_c)
