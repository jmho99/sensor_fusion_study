...
private:
    ...
    void keyboardCallback()
    {
        ...
        if (jmh_utils::keyboardAvailable())
        {
            ...
            else if (input == "p")
            {
                ...
                std::string data_dir = home_dir + "/sensor_fusion_study_ws/extrinsic_data";
                std::string pcd_path = data_dir + "/roi_points.pcd";
                // std::string plane_path = data_dir + "/ouster_plane";

                pubPlane(pcd_path, pcd_path, frame_);
            }
    ...
    void viewPcd(int frame)
    {
        ...
        crop.setMin(Eigen::Vector4f(0.0, -2.5, -3.0, 1.0));
        crop.setMax(Eigen::Vector4f(2.8, 0.0, 3.0, 1.0));
